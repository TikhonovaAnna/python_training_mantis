...
class SignupHelper:
    ...
    def new_user(self, username, email, password):
        ...
        # после нажатия на кнопку  отправляется пользователю письмо
        mail = self.app.mail.get_mail(username, password, "[MantisBT] Account registration")
        # из темы письма извлекаем ссылку
        url = self.extract_confirmation(mail)
    ...
    def extract_confirmation(self, text):
        # текст письма. Регулярное выражение
